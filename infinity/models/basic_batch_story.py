...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, ca_kv, infer_args):
        """
        ...
        if q_compact.dtype == torch.float32:    # todo: fp16 or bf16?
            ...
            oup = flash_attn_varlen_kvpacked_func(q=q_compact, kv=kv_compact, cu_seqlens_q=cu_seqlens_q, cu_seqlens_k=cu_seqlens_k, max_seqlen_q=Lq, max_seqlen_k=max_seqlen_k, dropout_p=0, softmax_scale=self.scale*infer_args['attn'][4], return_attn_probs = False).reshape(B, Lq, -1)
...
class CrossAttnBlock(nn.Module):
    ...
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0, 
                infer_args=None):    # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
        ...
        if self.fused_norm_func is None:
            ...
            x = x + self.ca(self.ca_norm(x), ca_kv, infer_args=infer_args).float().mul_(self.ca_gamma)
            x = x + self.drop_path(self.ffn( self.ln_wo_grad(x.float()).mul(scale2.add(1)).add_(shift2) ).mul(gamma2)) # this mul(gamma2) cannot be in-placed cuz we possibly use FusedMLP
        else:

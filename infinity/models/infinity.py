...
class Infinity(nn.Module):
    ...
    @torch.no_grad()
    def autoregressive_infer_cfg_batch_style(
        self,
        vae=None,
        scale_schedule=None,
        label_B_or_BLT=None,
        ...
        sampling_per_bits=1,
        feature_list=None,
        style_bit_indices=None
    ):   # returns List[idx_Bl]
        if g_seed is None: rng = None
        else: self.rng.manual_seed(g_seed); rng = self.rng
        assert len(cfg_list) >= len(scale_schedule)
        assert len(tau_list) >= len(scale_schedule)
        insert_style_attn = False
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            for block_idx, b in enumerate(self.block_chunks):
                ...
                for m in b.module:
                    last_stage = m(x=last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid, scale_ind=si, insert_style_attn=insert_style_attn)
                    if (cfg != 1) and (layer_idx in abs_cfg_insertion_layers):
                        # print(f'add cfg={cfg} on {layer_idx}-th layer output')
                        last_stage = cfg * last_stage[:B] + (1-cfg) * last_stage[B:] # cfg 적용
                        last_stage = torch.cat((last_stage, last_stage), 0)
            ...
            if vae_type != 0:
                ...
                idx_Bld_list.append(idx_Bld)
                if si in [0, 1, 2, 3, 4, 5, 6]:
                    idx_Bld[0] = style_bit_indices[si]
                codes = vae.quantizer.lfq.indices_to_codes(idx_Bld, label_type='bit_label') # [B, d, 1, h, w] or [B, d, 1, 2h, 2w]
                if si != num_stages_minus_1:
                    summed_codes += F.interpolate(codes, size=vae_scale_schedule[-1], mode=vae.quantizer.z_interplote_up)
                    if si == 0:
                        insert_style_alpha = 0
                        insert_style_attn = False
                    elif si == 1:
                        insert_style_alpha = 0
                        insert_style_attn = False
                        ...
                        insert_style_alpha = 1
                        insert_style_attn = False
                        summed_codes = self.abs_mean_std(summed_codes)
                    elif si == 4:
                        insert_style_alpha = 1
                        insert_style_attn = False
                        summed_codes = self.abs_mean_std(summed_codes)                    
                        ...
                        insert_style_alpha = 1
                        insert_style_attn = False                        
                    summed_codes[1:] = (1 - insert_style_alpha) * summed_codes[0].unsqueeze(0).expand_as(summed_codes[1:]) + insert_style_alpha * summed_codes[1:]
                    last_stage = F.interpolate(summed_codes, size=vae_scale_schedule[si+1], mode=vae.quantizer.z_interplote_up) # [B, d, 1, h, w] or [B, d, 1, 2h, 2w]
                    last_stage = last_stage.squeeze(-3) # [B, d, h, w] or [B, d, 2h, 2w]
                    if self.apply_spatial_patchify: # patchify operation
                        last_stage = torch.nn.functional.pixel_unshuffle(last_stage, 2) # [B, 4d, h, w]
